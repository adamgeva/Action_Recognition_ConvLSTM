...
class VideoFIFO:
    def __init__(self, config, time_depth, width, height):
        zero_frame = np.zeros([width, height, 3])
    ...
    def recover_block(self, human_bb, new_width, new_height):
        center_point = [0, 0]
        ...
        for i in range(self.time_depth):
            ...
            #cv2.waitKey(0)
            new_frame = self.fifo[12][int(center_point[0] - new_height // 2):int(center_point[0] + new_height // 2),
